...
def assess_novelty(test_text, match_text, similarity_score, test_filename="test", match_filename="match"):
    ...
    title1, abstract1, claim1 = map(lambda x: truncate_text(x, 300), extract_relevant_sections(test_text, test_filename))
    title2, abstract2, claim2 = map(lambda x: truncate_text(x, 300), extract_relevant_sections(match_text, match_filename))
...
A higher score indicates high degree of similarity.

Please determine if the test document appears to be:
(a) Clearly novel
...
Provide the most appropriate option.
"""
...
if __name__ == "__main__":
    ...
    test_doc = extract_text_from_pdf("US10839017_Unique.pdf")
